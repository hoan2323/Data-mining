...
# =========================
def _build_return(html_res, new_memory, chat_history, ui_history, has_results, session_products):
    """Helper tạo tuple return chuẩn cho chat_logic."""
    if has_results:
    ...
    return (
        ...
        res_col_update,
        chat_col_update,
        gr.update(visible=False),  # hero_title
...
def chat_logic(new_query, memory, chat_history, ui_history, session_products):
    ...
    # ✅ CASE 1: User tham chiếu số thứ tự ("máy 1", "chi tiết máy 1", "so sánh 1 và 2")
    ...
    if indices and session_products:
        ...
        if valid:
            has_results = True
            html_res    = render_html_results(valid)
            llm_context = format_lookup_context(valid[0]) if len(valid) == 1 else format_search_context(valid)
            ...
            # Giữ nguyên session_products (không reset)
            return _build_return(html_res, memory, chat_history, ui_history, has_results, session_products)

        else:
            # Có indices nhưng không khớp với session hiện tại → báo lỗi
            llm_context = "Không tìm thấy máy theo số thứ tự đó trong danh sách vừa tìm. Vui lòng tìm kiếm lại."
            bot_response = ask_llm(new_query, llm_context, chat_history)
            chat_history.append({"user": new_query, "assistant": bot_response})
            ui_history.append({"role": "user",      "content": new_query})
            ui_history.append({"role": "assistant", "content": bot_response})
            return _build_return("", memory, chat_history, ui_history, False, session_products)

    # ✅ CASE 2: Không phải index reference → chạy search bình thường
    def do_search(query):
        ...
        if "so sánh" in query_lower:
            ...
            if match:
                ...
                if combined_results:
                    return True, render_html_results(combined_results), format_search_context(combined_results), combined_results
    ...
    has_results, html_res, llm_context, found_list = do_search(combined_query)
    new_memory = combined_query

    if not has_results and memory:
        has_results, html_res, llm_context, found_list = do_search(new_query)
        new_memory = new_query if has_results else memory

    # ✅ Cập nhật session_products sau mỗi lần search mới
    if has_results and found_list:
        session_products = {i + 1: r for i, r in enumerate(found_list)}
    ...
    return _build_return(html_res, new_memory, chat_history, ui_history, has_results, session_products)
...
def reset_all():
    return (
        ...
        gr.update(elem_classes=["chat-col", "center"]),  # chat_col
        gr.update(visible=True),                   # hero_title
